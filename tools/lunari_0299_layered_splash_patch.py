...
if old_route in screens:
    ...
    raise SystemExit("0.2.9.11 Home route marker not found")
...
for marker in (
    ...
    if marker not in main:
        raise SystemExit(f"0.2.9.11 MainActivity marker missing: {marker}")
...
for marker in (
    ...
    "drawPlaced(TRAIL1_0299",
    "drawPlaced(TRAIL2_0299",
    "drawPlaced(TRAIL3_0299",
    "drawPlaced(TRAIL4_0299",
    "makeSettledDust0299()",
    ...
    if marker not in splash:
        raise SystemExit(f"0.2.9.11 splash marker missing: {marker}")

# IMPORTANT: the uploaded atlas is 1874x1809 FULL resolution.
# Keep these source rectangles exact. 0.2.9.11 changes only destination placement,
# scale, alpha and storyboard timing; it never halves or rewrites atlas geometry.
SPLASH_KT.write_text(splash, encoding="utf-8")

# User has installed 0.2.9.10/code22. This visual correction must be higher.
g = BUILD_GRADLE.read_text(encoding="utf-8")
if g.count("versionCode = 17") != 1 or g.count('versionName = "0.2.9.5"') != 1:
    raise SystemExit("0.2.9.11 version bump: expected accepted build-time 0.2.9.5 state not found")
g = g.replace("versionCode = 17", "versionCode = 23", 1)
g = g.replace('versionName = "0.2.9.5"', 'versionName = "0.2.9.11"', 1)
BUILD_GRADLE.write_text(g, encoding="utf-8")

print("Lunari 0.2.9.11 reference-matched storyboard splash applied")
